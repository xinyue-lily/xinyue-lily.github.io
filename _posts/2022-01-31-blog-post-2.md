...
## 1. Flask g
Flask provides the *g object* for the application context to store common data during a request or CLI command. 

The g name stands for “global”, It is a simple namespace object that has the same lifetime as an application context.
...
•	g is used to transmit the data among different web pages. 
...
• Firstly, we check whether there is a database called message_db in the g attribute of the app. If not, then connect to that database.

• We check whether a table called messages exists in message_db, if there is no table available, we create the table messages with three columns: ID as an integer, handle as text, and message as text.
...
（2) The second function we created is called insert_message() which should handle inserting a user message into the database of messages.

• We extract the message and the handle from request if they exist. We should ensure submit.html template creates these fields from user input. 

• Then we connect the database using cursor.

• To ensure that the ID number of each message is unique, we set the ID number equal to one plus the current number of rows.
...
• We connect the database using cursor.
...
(3)	When we press the "view messages" button, the program will use the function random_messages(n) from app.py to return n messages from table messages (the number n is inputted by the user) and record it into g.result. 
