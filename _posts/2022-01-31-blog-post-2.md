...
•	We will fetch a function called random_messges(n) from db_db_app.py to pick n messages from the database randomly.
...
•	If they exist, we will fetch a function called insert_message from db_db_app.py.
...
def submit():
    if request.method == 'GET':
        ...
        try:
            #fetch a function called insert_message from db_db_app.py. 
