...
def plot(X,label):
    ...
    plt.title('The correct rate:{:.0%}'.format(max(1000-correct.sum(),correct.sum())/1000))
    #plt.show()   
...
def correct_rate(X,y,label):
    ...
    """
    correct = np.isclose(y,label)
    return 'the correct rate:{:.0%}'.format(max(1000-correct.sum(),correct.sum())/1000)
